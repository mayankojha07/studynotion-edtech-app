...
function CourseDetails() {
  ...
  const isAdded = cart?.findIndex(
    (item) => item._id === response?.data?.courseDetails?._id
  );
  const [addedToCart, setAddedToCart] = useState(isAdded === -1 ? false : true);
  // setAddedToCart(isAdded);

  const [avgReviewCount, setAvgReviewCount] = useState(0);
  useEffect(() => {
    console.log("Course and review -> ", response?.data);
  ...
  const handleRemoveFromCart = () => {
    if (token) {
      dispatch(removeFromCart(response?.data?.courseDetails?._id));
